...
function vote(){
    ...
    $para1 = filter_var($_REQUEST['para1'], FILTER_SANITIZE_NUMBER_INT);
    $para2 = filter_var($_REQUEST['para2'], FILTER_SANITIZE_NUMBER_INT);

    if (get_post_status($para1) == false) {
        die();
    }
...
add_action("after_switch_theme", "createUpvotestable");

function createUpvotestable(){
    global $wpdb;
...
// Checks if user has voted for post
